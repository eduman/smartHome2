...
class TimeShiftControlStrategy(AbstractControlStategy):
	def __init__(self):
		strategyName =  (inspect.stack()[0][1]).replace(".py", "").replace("./", "") #os.path.realpath(__file__) 
		super(TimeShiftControlStrategy, self).__init__(strategyName, logLevel)
		try:
...
#			else:		
			...
			self.context = Context(self.strategyName)
	...
	def setRuleEngine(self):
		...
		timeShift = TimeShiftActionRule(self.context, self.logger, self.mqtt)
		self.ruleEngine.addRule(timeShift)
